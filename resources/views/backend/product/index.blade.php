...
$(document).ready(function () {
    ...
    var $table = $('#product-table').DataTable({
         ...
         stateSave: true,
         dom: '<"toolbar">rtp',
         ajax: '{!! route('product.source',$menu_id) !!}',
         columns: [
      ...
      $('#product-table_wrapper > div.toolbar').html('<div class="row">' +
                ...
                '</div>'+
                '<div class="col-lg-2">'+
                    '<a href="{{ route("product.create",$menu_id) }}" class="btn btn-sm btn-primary float-right" data-toggle="tooltip" title="Tambah Data"><i class="fas fa-plus"></i></a>'+
                '</div>' +
                '</div>');
