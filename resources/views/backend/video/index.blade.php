...
<div class="col-lg-12">
    <div class="card shadow-sm mb-4">
        ...
        <div class="card-body">
            <table class="table table-sm table-bordered table-striped" id="video-table">
                <thead>
                    <tr>
                        <th>#</th>
                        <th>Nama</th>
...
$(document).ready(function () {
    ...
    var $table = $('#video-table').DataTable({
         ...
         stateSave: true,
         dom: '<"toolbar">rtp',
         ajax: '{!! route('video.source',$menu_id) !!}',
         columns: [
            ...
            {data: 'url', name: 'url',width:"10%", orderable : true},
            {data: 'action', name: 'action',width:"5%", orderable : false}
         ]
     });

      $('#video-table_wrapper > div.toolbar').html('<div class="row">' +
                '<div class="col-lg-10">'+
                ...
                '</div>'+
                '<div class="col-lg-2">'+
                    '<a href="{{ route("video.create",$menu_id) }}" class="btn btn-primary float-right" data-toggle="tooltip" title="Tambah Data"><i class="fas fa-plus"></i></a>'+
                '</div>' +
                '</div>');
    ...
    $('#video-table').on('click','a.delete-data',function(e) {
        e.preventDefault();
